...
class BaseVectorEnv(object):
    ...
    def latency_bin_packing_chunk_step(
        ...
        *,
        ...
        Each bin owns one CPU core group and runs at most one env at a time.
        Bins execute in parallel, while envs assigned to the same bin execute
        their full action chunks serially.
        """
        self._assert_is_not_closed()
        id = list(self._wrap_id(id))
        if self.is_async:
