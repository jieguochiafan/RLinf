...
def get_env_cls(env_type: str, env_cfg=None):
    ...
    if env_type == SupportedEnvType.MANISKILL:
        if env_cfg.get("enable_offload", False):
            from rlinf_rollout.envs.maniskill.maniskill_offload_env import (
                ManiskillOffloadEnv,
            )

            return ManiskillOffloadEnv
        else:
            from rlinf_rollout.envs.maniskill.maniskill_env import ManiskillEnv

            return ManiskillEnv
    elif env_type == SupportedEnvType.MANISKILL_RLT:
        from rlinf_rollout.envs.maniskill.maniskill_rlt_env import ManiskillRLTEnv

        return ManiskillRLTEnv
    elif env_type == SupportedEnvType.LIBERO:
        from rlinf_rollout.envs.libero.libero_env import LiberoEnv

        return LiberoEnv
    elif env_type == SupportedEnvType.ROBOTWIN:
        from rlinf_rollout.envs.robotwin.robotwin_env import RoboTwinEnv

        return RoboTwinEnv
    elif env_type == SupportedEnvType.ISAACLAB:
        from rlinf_rollout.envs.isaaclab import REGISTER_ISAACLAB_ENVS
        ...
        from rlinf_rollout.envs.metaworld.metaworld_env import MetaWorldEnv

        return MetaWorldEnv
    elif env_type == SupportedEnvType.BEHAVIOR:
        from rlinf_rollout.envs.behavior.behavior_env import BehaviorEnv

        return BehaviorEnv
    elif env_type == SupportedEnvType.CALVIN:
        from rlinf_rollout.envs.calvin.calvin_gym_env import CalvinEnv

        return CalvinEnv
    elif env_type == SupportedEnvType.ROBOCASA:
        from rlinf_rollout.envs.robocasa.robocasa_env import RobocasaEnv

        return RobocasaEnv
    elif env_type == SupportedEnvType.ROBOCASA365:
        from rlinf_rollout.envs.robocasa365.robocasa365_env import Robocasa365Env

        return Robocasa365Env
    elif env_type == SupportedEnvType.REALWORLD:
        from rlinf_rollout.envs.realworld import RealWorldEnv

        return RealWorldEnv
    elif env_type == SupportedEnvType.HABITAT:
        from rlinf_rollout.envs.habitat.habitat_env import HabitatEnv

        return HabitatEnv
    elif env_type == SupportedEnvType.FRANKASIM:
        from rlinf_rollout.envs.frankasim.frankasim_env import FrankaSimEnv

        return FrankaSimEnv
    elif env_type == SupportedEnvType.GENESIS:
        from rlinf_rollout.envs.genesis.genesis_env import GenesisEnv

        return GenesisEnv
    elif env_type == SupportedEnvType.OPENSORAWM:
        from rlinf_rollout.envs.world_model.world_model_opensora_env import OpenSoraEnv

        return OpenSoraEnv
    elif env_type == SupportedEnvType.WANWM:
        from rlinf_rollout.envs.world_model.world_model_wan_env import WanEnv

        return WanEnv
    elif env_type == SupportedEnvType.EMBODICHAIN:
        from rlinf_rollout.envs.embodichain.embodichain_env import EmbodiChainEnv

        return EmbodiChainEnv
    elif env_type == SupportedEnvType.ROBOVERSE:
        from rlinf_rollout.envs.roboverse.roboverse_env import RoboVerseEnv

        return RoboVerseEnv
    elif env_type == SupportedEnvType.D4RL:
        from rlinf_rollout.envs.d4rl.d4rl_env import D4RLEnv

        return D4RLEnv
    elif env_type == SupportedEnvType.POLARIS:
        from rlinf_rollout.envs.polaris.polaris_env import PolarisEnv
