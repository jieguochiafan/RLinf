...
class ClusterEnvVar(str, Enum):
    ...
    """Enable shipping the ``rlinf_rollout`` Python package to workers via Ray ``runtime_env`` (``py_modules``).

    Only the ``rlinf_rollout/`` package directory is packaged (not tests, docs, etc.).

    Values (``RLINF_CODE_WORKING_DIR``):
        - Unset / ``0`` / ``false`` / ``off`` / ``no``: disabled (same as legacy behavior: no Ray code sync).
        - ``auto``: infer the package directory from the imported ``rlinf_rollout`` package.
        - Absolute path: the ``rlinf_rollout`` package dir, or a checkout root containing
          ``pyproject.toml`` and ``rlinf_rollout/``.
...
class Cluster:
    ...
    SYS_NAME = "RLinf"
    """Prefix of every scheduler env var (``RLINF_*``) and the Ray namespace.

    Kept identical to the training-side RLinf so that ``RLINF_NODE_RANK`` and
    friends keep working when a rollout service is launched from an existing
    RLinf deployment.
    """

    PACKAGE_NAME = "rlinf_rollout"
    """Python package shipped to Ray workers by the code-sync runtime env."""
        ...
        try:
            ...
            if self._ray_code_sync_fragment is not None:
                ...
                self._logger.info(
                    ...
                    "only the rlinf_rollout package from the launch node. Disable with %s=0.",
                    Cluster.SYS_NAME,
                    tuple(py_mods),
    ...
    def modify_profile_context(
        ...
        if profiling_cfg.output_dir is None:
            ...
            from rlinf_rollout.utils.logging import get_logger
    ...
    @classmethod
    def _infer_package_dir_for_ray_working_dir(cls) -> Path:
        """Locate the installed ``rlinf_rollout`` package directory."""
        import rlinf_rollout

        pkg_dir = Path(rlinf_rollout.__file__).resolve().parent
        if not (pkg_dir / "__init__.py").is_file():
            raise RuntimeError(
                f"{cls.SYS_NAME} could not locate the {cls.PACKAGE_NAME} package for "
                f"{cls.get_full_env_var_name(ClusterEnvVar.CODE_WORKING_DIR)}=auto "
                f"(no __init__.py under {pkg_dir}). Set the variable to an absolute "
                "path of the package or its checkout on the launch node."
            )
        return pkg_dir
    ...
    @classmethod
    def _resolve_explicit_abs_path_to_repo_and_package(
        cls,
        abs_path: Path,
        env_var_key: str,
    ) -> tuple[Path, Path]:
        """Split an explicit path into ``(root to strip, package dir to ship)``."""
        resolved = abs_path.expanduser().resolve()
        if not resolved.is_dir():
            raise FileNotFoundError(
                f"{env_var_key} points to a non-directory path: {resolved}"
            )
        if resolved.name == cls.PACKAGE_NAME and (resolved / "__init__.py").is_file():
            return resolved.parent, resolved
        if (resolved / "pyproject.toml").is_file() and (
            resolved / cls.PACKAGE_NAME / "__init__.py"
        ).is_file():
            return resolved, resolved / cls.PACKAGE_NAME
        raise RuntimeError(
            f"{env_var_key}={resolved}: expected an absolute path to the "
            f"{cls.PACKAGE_NAME} package directory, or to a checkout root "
            f"containing pyproject.toml and {cls.PACKAGE_NAME}/__init__.py."
        )
    ...
    def _prepare_ray_code_sync_runtime_env_fragment(
        ...
        """Build Ray ``runtime_env`` with ``py_modules`` for the rollout package only."""
        ...
        if raw == "":
            return None, ()
        if lowered == "auto":
            pkg_dir = cls._infer_package_dir_for_ray_working_dir()
            ...
            repo_root, pkg_dir = cls._resolve_explicit_abs_path_to_repo_and_package(
                path_obj, env_key
            )

        if not (pkg_dir / "__init__.py").is_file():
            raise FileNotFoundError(
                f"{cls.PACKAGE_NAME} package missing or invalid at {pkg_dir}."
            )
        py_mod_path = pkg_dir.resolve()
        fragment: dict[str, Any] = {"py_modules": [str(py_mod_path)]}
        strip_roots = {
            os.path.realpath(str(repo_root)),
            os.path.realpath(str(pkg_dir)),
