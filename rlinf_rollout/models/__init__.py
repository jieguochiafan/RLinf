...
def _register_builtin_models():
    def _build_openvla(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.openvla import get_model

        return get_model(cfg, torch_dtype)

    def _build_openvla_oft(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.openvla_oft import get_model

        return get_model(cfg, torch_dtype)

    def _build_openpi(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.openpi import get_model

        return get_model(cfg, torch_dtype)

    def _build_openpi_pytorch(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.openpi_pytorch import get_model

        return get_model(cfg, torch_dtype)

    def _build_dexbotic_pi(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.dexbotic_pi import get_model

        return get_model(cfg, torch_dtype)

    def _build_dexbotic_dm0(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.dexbotic_dm0 import get_model

        return get_model(cfg, torch_dtype)

    def _build_mlp_policy(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.mlp_policy import get_model
    ...
    def _build_gr00t(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.gr00t import get_model

        return get_model(cfg, torch_dtype)

    def _build_cnn_policy(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.cnn_policy import get_model

        return get_model(cfg, torch_dtype)

    def _build_flow_policy(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.flow_policy import get_model

        return get_model(cfg, torch_dtype)

    def _build_lingbotvla(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.lingbotvla import get_model

        return get_model(cfg, torch_dtype)

    def _build_abot_m0(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.abot_m0 import get_model

        return get_model(cfg, torch_dtype)

    def _build_starvla(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.starvla import get_model

        return get_model(cfg, torch_dtype)

    def _build_dreamzero(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.dreamzero import get_model
    ...
    def _build_openpi_cfg(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.openpi_cfg import get_model

        return get_model(cfg, torch_dtype)

    def _build_recap_value_model(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.value_model.recap import get_model

        return get_model(cfg, torch_dtype)

    def _build_steam_value_model(cfg: DictConfig, torch_dtype):
        from rlinf_rollout.models.embodiment.value_model.steam import get_model
