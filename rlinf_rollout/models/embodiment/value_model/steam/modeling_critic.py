...
``rlinf_rollout.models.embodiment.value_model.recap.modeling_critic`` (same keys in
...
class CriticOutput(ModelOutput):
    ...
    Field list deliberately matches :class:`~rlinf_rollout.models.embodiment.value_model.recap.\
modeling_critic.CriticOutput` so worker code stays duck-type-compatible.
    ...
    :class:`~rlinf_rollout.models.embodiment.value_model.steam.\
...
class SteamCriticModel(nn.Module):
    ...
    concat MLP) with the RLinf-side observation/forward contract:
        ...
        - ``observation`` is a dict in the same format produced by
          :class:`~rlinf_rollout.models.embodiment.value_model.recap.data_collator.\
ValueDataCollator`: ``images: dict[cam_name, Tensor[B,3,H,W]]`` in [0, 1],
    ...
    def _compute_loss(self, logits, bin_labels):
        ...
        Paired with :class:`~rlinf_rollout.data.datasets.steam.PairDataset`,
    ...
    def _predicted_signed_value(self, probs: Tensor) -> Tensor:
        ...
        :func:`~rlinf_rollout.data.datasets.steam.binning.bin_centers`.
        """
        num_bins = int(self.config.num_bins)
        half = num_bins // 2
        if half < 1:
