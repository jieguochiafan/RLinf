...
Ported from the training repo's ``rlinf/workers/rollout/vllm/vllm_worker.py``, with
every trainer coupling removed:

* ``cfg.algorithm.sampling_params`` -> ``cfg.rollout.sampling_params``.
* ``cfg.actor.tokenizer.trust_remote_code`` -> ``cfg.rollout.model.trust_remote_code``.
* ``cfg.runner.seq_length`` -> ``cfg.rollout.max_model_len``.
...
* ``ModelParallelComponentPlacement`` ->
  :class:`~rlinf_rollout.utils.rollout_placement.RolloutComponentPlacement`.

Sleep/wake offload (``enable_sleep_mode`` + ``collective_rpc("offload_model_weights")``)
and the dynamic-scaling hooks are kept as-is.
"""
...
from rlinf_rollout.data.io_struct import RolloutRequest, RolloutResult, SeqGroupInfo
from rlinf_rollout.engines.vllm import VLLMExecutor
...
from rlinf_rollout.scheduler.dynamic_scheduler.utils import get_scheduler_channel
from rlinf_rollout.utils.data_process import process_image_data
...
class VLLMWorker(Worker):
    ...
    def __init__(
        ...
        weight_reload: Literal["sync", None] = "sync",
    ):
        Worker.__init__(self)
        self._cfg = config
        self._placement = placement
        self.weight_reload = weight_reload
        self._rollout_cfg = LLMRolloutConfig.from_dictconfig(config, validate=False)
    ...
    def build_weight_sync_setup(self) -> Optional[EngineWeightSyncSetup]:
        ...
        Returns:
            The setup handed to each vLLM worker process, or ``None`` when this
    ...
    def _load_tokenizer(self):
        ...
        trust_remote_code = self._rollout_cfg.trust_remote_code
        try:
    ...
    def _get_sampling_params_from_config(self) -> SamplingParams:
        ...
        """
        cfg_sampling_params = self._cfg.rollout.sampling_params
    ...
    async def init_worker(self) -> None:
        ...
        ready to use parameters pushed by the weight sender.
        """
        engine_args: EngineArgs = EngineArgs(
            ...
            trust_remote_code=self._rollout_cfg.trust_remote_code,
            max_model_len=self._rollout_cfg.max_model_len,
        ...
        executor_class = partial(
            ...
            weight_sync_setup=self.build_weight_sync_setup(),
            enforce_eager=bool(self._cfg.rollout.enforce_eager),
            dp_rank=self._rank,
        )
