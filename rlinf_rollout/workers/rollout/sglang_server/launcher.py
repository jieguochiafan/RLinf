...
Vendored from the training repo's ``rlinf/workers/rollout/sglang_server/launcher.py``.
It never reads a trainer config section: the ``config`` argument is forwarded to the
worker groups verbatim, and everything the launcher itself consumes lives in
``router_server_args`` (typically ``cfg.rollout``).
...
from rlinf_rollout.scheduler.placement import PlacementStrategy
...
def launch_sglang_router_and_server(
    ...
    *,
    ...
    Args:
        config: Rollout-system ``DictConfig`` passed through to the worker
            groups for cluster/runtime context.
        cluster: Active :class:`rlinf_rollout.scheduler.Cluster`.
