...
from rlinf_rollout.utils.http_client import no_proxy_env
...
class SGLangServerWorker(Worker):
    ...
    Args:
        config: Rollout-system ``DictConfig``. Kept for parity with other
        ...
        sglang_cfg: The sub-config block whose keys are forwarded verbatim
            ...
            Typically ``cfg.rollout.server`` when used inside a rollout,
