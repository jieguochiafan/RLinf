...
from rlinf_rollout.config import LLMRolloutConfig
from rlinf_rollout.data.io_struct import (
    RolloutResult,
)
from rlinf_rollout.scheduler import Channel, Worker

__all__ = ["ServerRolloutWorker", "TrainingDataStorage"]
...
class ServerRolloutWorker(Worker):
    ...
    - Compatible with an external online-RL driver

    Config mapping relative to the training repo: ``rollout_server.tracking_rollout``
    -> ``rollout.tracking_server``, ``algorithm.sampling_params`` ->
    ``rollout.sampling_params``, ``data.rollout_batch_size * algorithm.group_size`` ->
    ``rollout.batch_size * rollout.group_size``, ``runner.seq_length`` ->
    ``rollout.max_model_len``.

    Args:
        cfg: Rollout-system config with ``rollout.kind: llm``.
    """
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self._cfg = cfg
        self._rollout_cfg = LLMRolloutConfig.from_dictconfig(cfg, validate=False)
        ...
        # Configuration
        server_cfg = cfg.rollout.tracking_server
        self._server_host = server_cfg.get("host", "0.0.0.0")
        self._server_port = server_cfg.get("port", 8082)
        self._enable_dummy_data = server_cfg.get("enable_dummy_data", False)

        # Unified data source for both HTTP and Channel data
        self._data_source = asyncio.Queue()

        # Initialize training data storage
        storage_config = server_cfg.get("storage", None)
        if storage_config is not None:
            storage_config = dict(storage_config)
        self._storage = TrainingDataStorage(storage_config)

        # Processing configuration
        self._max_new_tokens = self._rollout_cfg.sampling_params_cfg.get(
            "max_new_tokens", 512
        )
        max_model_len = self._rollout_cfg.max_model_len
        assert max_model_len is not None, (
            "rollout.max_model_len must be set for the tracking server: it bounds the "
            "prompt truncation length (it replaces the trainer's runner.seq_length)."
        )
        assert max_model_len > self._max_new_tokens, (
            f"rollout.max_model_len ({max_model_len}) must exceed "
            f"rollout.sampling_params.max_new_tokens ({self._max_new_tokens})."
        )
        self._max_prompt_length = max_model_len - self._max_new_tokens
        self._batch_size = self._rollout_cfg.total_tasks
    ...
    def _convert_training_data_to_rollout_result(
        ...
        group_size = self._rollout_cfg.group_size
