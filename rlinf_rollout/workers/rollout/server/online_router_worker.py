...
__all__ = ["CompleteRequest", "CompleteResponse", "OnlineRouterWorker"]
...
class OnlineRouterWorker(Worker):
    """Online router worker with a FastAPI ``/v1/completions`` endpoint.

    Fans requests out across the engine group and gates them while weights are being
    replaced, so an in-flight completion never straddles two policy versions.

    Config mapping relative to the training repo: ``rollout_server.online_router`` ->
    ``rollout.online_router``, ``algorithm.sampling_params`` ->
    ``rollout.sampling_params``.

    Args:
        cfg: Rollout-system config with ``rollout.kind: llm``.
        placement: Rollout-only placement; only ``rollout_dp_size`` is used.
    """

    def __init__(self, cfg: DictConfig, placement: RolloutComponentPlacement):
        Worker.__init__(self)

        self._cfg = cfg

        # Configuration
        router_cfg = cfg.rollout.online_router
        self._server_host = router_cfg.get("host", "0.0.0.0")
        self._server_port = router_cfg.get("port", 8081)
        self._rollout_instance_num = placement.rollout_dp_size
        sampling_params_cfg = cfg.rollout.sampling_params
        self._sampling_params = SGLangWorker.get_sampling_param_from_config(
            sampling_params_cfg
        )
        if "stop" in sampling_params_cfg:
            self._sampling_params["stop"] = sampling_params_cfg["stop"]
