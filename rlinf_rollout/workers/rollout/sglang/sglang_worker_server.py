...
"""SGLang worker that additionally exposes an OpenAI-compatible HTTP endpoint.

Ported verbatim from the training repo's
``rlinf/workers/rollout/sglang/sglang_worker_server.py`` except for the placement
type and the config keys, which now come from the rollout system's own schema
(``rollout.sglang.server.{host,port}``).
"""
...
__all__ = ["SGLangWorkerWithHTTPServer"]
...
class SGLangWorkerWithHTTPServer(SGLangWorker):
    """An :class:`SGLangWorker` with a per-engine ``/v1/chat/completions`` endpoint.

    Args:
        config: Rollout-system config with ``rollout.kind: llm``.
        placement: Rollout-only placement describing this engine's accelerators.
        weight_reload: See :class:`SGLangWorker`.
        config_rollout: Optional override of the ``rollout`` sub-config.
        http_server_host: Fallback bind host when ``rollout.sglang.server.host`` is
            unset.
        http_server_port: Fallback base port when ``rollout.sglang.server.port`` is
            unset; the worker's rank is added to it.
    """
    ...
    def __init__(
        ...
        server_cfg = (self._cfg_rollout.get("sglang") or {}).get("server") or {}
    ...
    def http_server_start(self):
        """Start serving the OpenAI-compatible endpoint in a background task."""
    ...
    async def http_server_stop(self):
        """Stop the HTTP server and await its task."""
    ...
    async def init_worker(self):
        """Bring up the engine, then start the HTTP endpoint."""
    ...
    def get_server_address(self) -> str:
        """Return the reachable ``host:port`` of this worker's HTTP endpoint."""
        host = self._http_server_host
        if host == "0.0.0.0":
            import ray.util
