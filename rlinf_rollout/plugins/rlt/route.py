...
from rlinf_rollout.plugins.rlt.expert import predict_expert_actions
from rlinf_rollout.plugins.rlt.transition import use_simulator_transition_replay
...
def build_rlt_route(cfg: Any) -> RLTRoute:
    """Build the RLT routing policy from ``rollout.plugins.rlt.schedule``."""
    if use_simulator_transition_replay(cfg):
        from omegaconf import OmegaConf

        schedule_cfg = (
            OmegaConf.select(cfg, "rollout.plugins.rlt.schedule", default=None) or {}
        )
