...
from rlinf_rollout.scheduler import Worker, WorkerAddress
...
class Scheduler(_Scheduler):
    ...
    A Scheduler is a Task that manages the TP worker, and performs necessary weight
    synchronization with an external weight sender plus weight offloading. The
    sender's identity and rank layout arrive in an
    :class:`~rlinf_rollout.weight_sync.llm.EngineWeightSyncSetup`; nothing here reads
    a trainer config section.
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set by init_rlinf_worker(); no weight source is known before that.
        self.weight_reload: Optional[str] = None
        self._weight_setup: Optional[EngineWeightSyncSetup] = None
        self._weight_source_group_name: Optional[str] = None
        self._weight_source_rank: Optional[int] = None
    ...
    def batch_load_hf_weight(self, state_dict: dict[str, Any]) -> Any:
        ...
        model = self.tp_worker.worker.model_runner.model
        batch_weight = []
        if self._weight_setup.is_collocated:
    ...
    def sync_hf_weight(self, recv_req: SyncHFWeightInput):
        ...
        assert not self._enforce_eager, (
            "sglang weight sync requires cuda graph capture (enforce_eager=False), "
            "because it relies on the memory saver."
        )
    ...
    def init_rlinf_worker(
        ...
        weight_sync_setup: Optional[EngineWeightSyncSetup] = None,
        enforce_eager: bool = False,
    ):
        """Attach this engine shard to the rollout worker tree.
        ...
        Args:
            ...
            weight_sync_setup: Sender topology + rollout parallel sizes, required
                when ``weight_reload == "sync"``. Replaces the training repo's
                ``(placement, trainer_cfg)`` pair.
            enforce_eager: Mirror of ``rollout.enforce_eager``; weight sync asserts
                it is ``False`` because it relies on SGLang's memory saver.
        """
        ...
        if weight_reload == "sync":
            assert weight_sync_setup is not None, (
                "weight_sync_setup is required when weight_reload='sync'; it carries "
                "the weight sender's group name and rank layout."
            )
            ...
            model = self.tp_worker.worker.model_runner.model
            # it's important to use load_weight to load resharded weight from a
            # pre-sharded sender (e.g. Megatron); a full-tensor sender (e.g. FSDP)
            # must keep the upstream loader path.
            for _, module in model.named_modules():
                if hasattr(module, "use_presharded_weights"):
                    module.use_presharded_weights = weight_sync_setup.presharded_weights

            if weight_sync_setup.validate_first_sync:
                self.weight_norm_dict = validate_weight_init(model)

            self._rlinf_worker.log_info(
                f"Running Scheduler dp rank {self._rlinf_worker.get_parent_rank()}, "
                f"tp rank {self.tp_rank}, weight source "
                f"{self._weight_source_group_name}:{self._weight_source_rank}"
            )
...
def run_scheduler_process(*args, **kwargs):
    from rlinf_rollout.utils.patcher import Patcher

    Patcher.clear()
    Patcher.add_patch(
        "sglang.srt.managers.scheduler.Scheduler",
        "rlinf_rollout.engines.sglang.common.sgl_scheduler.Scheduler",
    )
    Patcher.apply()
