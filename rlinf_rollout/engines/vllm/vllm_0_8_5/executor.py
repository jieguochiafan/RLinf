...
from rlinf_rollout.scheduler.manager.worker_manager import WorkerAddress
...
class VLLMExecutor(MultiprocExecutor):
    """Multiproc executor that hands each worker its weight-sync setup.

    Args:
        vllm_config: Upstream vLLM config.
        dp_rank: Rank of this engine among the rollout engine processes.
        ...
        weight_sync_setup: Sender topology + rollout parallel sizes, or ``None`` for a
            ...
            ``(rlinf_config, placement)`` pair.
        enforce_eager: Mirror of ``rollout.enforce_eager``.
    """
    ...
    def __init__(
        ...
        self.weight_sync_setup = weight_sync_setup
        ...
        try:
            for rank in range(self.world_size):
                worker = VLLMWorkerProc.make_worker_process(
                    ...
                    weight_sync_setup=self.weight_sync_setup,
...
class VLLMWorkerProc(WorkerProc):
    def __init__(
        ...
        all_kwargs[rank] = {
            ...
            "weight_sync_setup": weight_sync_setup,
